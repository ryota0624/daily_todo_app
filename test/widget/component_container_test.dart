...
void main() {
  ...
  group('container.resolve', () {
    ...
    test('resolve generic class', () {
      final generic = container
          .addT<GenericClass<ComponentB>>(GenericClass())
          .resolve<GenericClass<ComponentB>>();
      expect(generic.getMessage(), equals('message'));
    });

    test('resolve abstract generic class', () {
      final generic = container
          .addT<AbstractGenericClass<int>>(GenericClassImplA())
          .resolve<AbstractGenericClass<int>>();
      expect(generic.getMessage(), equals('message3'));
    });

    test('called no register component', () {
      expect(() {
        final _ = container.resolve<NotFoundComponent>();
      }, throwsA(const TypeMatcher<ContainerBuildError>()));
    });
  });
  ...
  group('container.lazy', () {
    test(
        'if component had register by factory(), '
        'resolve method return every '
